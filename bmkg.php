...
$props = array(
    'Aceh', 'Bali', 'BangkaBelitung', 'Banten', 'Bengkulu', 'DIYogyakarta', 'DKIJakarta', 'Gorontalo', 'Jambi', 'JawaBarat',
...
$wilayah = $db->select("t_wilayah", "*", ["ORDER" => "propinsi"]);

//simpan ke file
file_put_contents("./wp-resources/cuaca/wilayah.json", json_encode($wilayah));

foreach ($wilayah as $wil) {
    $id = $wil['id'];
    $cuaca = $db->query('SELECT jamCuaca,kodeCuaca,cuaca,humidity,tempC,tempF FROM prefix_t_cuaca WHERE DATE(jamCuaca)>=DATE(NOW()) AND idWilayah=' . $id . ' ORDER BY jamCuaca ASC')->fetchAll(PDO::FETCH_ASSOC);

    //simpan ke file
    file_put_contents("./wp-resources/cuaca/$id.json", json_encode($cuaca));
}
